...
wxWindow *ControllerEditor::make_numbers_panel(wxWindow *parent) {
  ...
  outer_sizer->Add(header_text(p, TITLE_STR("Controller Number "))); // we need an EXTRA space
...
wxWindow *ControllerEditor::make_val_mapping_panel(wxWindow *parent) {
  ...
  value_sizer->Add(header_text(p, TITLE_STR("Value Mapping")));
...
void ControllerEditor::save(wxCommandEvent& _) {
  controller->set_cc_num(int_from_chars(cb_cc_number->GetValue()));
  controller->set_translated_cc_num(int_from_chars(cb_xlated_number->GetValue()));
  controller->set_filtered(cb_filtered->IsChecked());
